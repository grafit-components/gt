...
export class FormComponent implements OnInit {
  ...
  constructor(private notifications: ItskNotificationService, private data: DataService) {
    this.form = new FormGroup({
      well: new FormControl(this.new.well, [
        Validators.required
      ]),
      field: new FormControl(this.new.field, [
        Validators.required
      ]),
      layer: new FormControl(this.new.layer, [
        Validators.required
      ]),
      liq: new FormControl(this.new.liq, [
        Validators.min(0)
      ]),
      oil: new FormControl(this.new.oil, [
        Validators.min(0)
      ]),
      pressure: new FormControl(this.new.pressure, [
        Validators.min(0)
      ]),
      repair: new FormControl(this.new.repair, [
        WellDatesValidator.checkRepair(this.new.created)
      ]),
      created: new FormControl(this.new.created, [
        Validators.required
      ])
    });
  }
  ...
  save() {
    ...
    this.form.reset();
    this.notifications.add(new ItskNotification({
