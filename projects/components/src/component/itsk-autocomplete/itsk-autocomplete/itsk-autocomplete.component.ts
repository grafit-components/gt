        ...
        map(inputValue => {
            if (this.values?.length) {
                if (!inputValue) {
                    ...
                    this.items = this.values.filter(
                        item => item?.search(new RegExp(searchWithOutRegExp, 'i')) > -1);
                }
                ...
                this.height = itemsCount > this.itemScrollCount
                    ? this.itemSize * this.itemScrollCount
                    : this.itemSize * itemsCount;
                return this.items;
            }
            return null;
        }),
